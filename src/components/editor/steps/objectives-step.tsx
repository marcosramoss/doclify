...
import { Plus, Trash2, Target, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
import { objectivesSchema, type ObjectivesFormData } from '@/lib/validations/project';
...
const priorityOptions = [
  { value: 'high', label: 'Alta', color: 'bg-red-100 text-red-800', icon: AlertCircle },
  { value: 'medium', label: 'Média', color: 'bg-yellow-100 text-yellow-800', icon: Clock },
  { value: 'low', label: 'Baixa', color: 'bg-green-100 text-green-800', icon: CheckCircle },
];

const typeOptions = [
  { value: 'business', label: 'Negócio', description: 'Objetivos relacionados ao negócio e ROI' },
  { value: 'technical', label: 'Técnico', description: 'Objetivos técnicos e de arquitetura' },
  { value: 'user', label: 'Usuário', description: 'Experiência e satisfação do usuário' },
  { value: 'performance', label: 'Performance', description: 'Métricas de desempenho e velocidade' },
  { value: 'security', label: 'Segurança', description: 'Objetivos de segurança e compliance' },
...
export function ObjectivesStep({ onNext, onBack }: ObjectivesStepProps) {
  ...
  const addCommonObjective = (type: string, title: string) => {
    const exists = watchedObjectives.some(obj => 
  ...
  const getObjectivesByType = (type: string) => {
    return watchedObjectives.filter(obj => obj.type === type);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Define os objetivos do projeto, critérios de sucesso e possíveis limitações.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-6">
            {typeOptions.map((type) => {
              const count = getObjectivesByType(type.value).length;
              return (
          ...
          {typeOptions.map((type) => {
            const typeObjectives = getObjectivesByType(type.value);
            const commonObjs = commonObjectives[type.value as keyof typeof commonObjectives] || [];
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {commonObjs.length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {commonObjs.map((obj) => {
                            const exists = watchedObjectives.some(o => 
                              o.title.toLowerCase() === obj.toLowerCase()
                            );
                            return (
                              <Button
                                key={obj}
                                ...
                                size="sm"
                                onClick={() => addCommonObjective(type.value, obj)}
                                disabled={exists}
                                className="h-auto p-3 text-left justify-start"
                              >
                                {exists && <CheckCircle className="h-4 w-4 mr-2 flex-shrink-0" />}
                    ...
                    <div className="space-y-4">
                      {fields.map((field, index) => {
                        if (watchedObjectives[index]?.type !== type.value) return null;
                        
                        const priorityInfo = getPriorityInfo(watchedObjectives[index]?.priority || 'medium');
                          ...
                          <Card key={field.id} className="p-4 bg-gray-50">
                            <div className="space-y-4">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  <Label htmlFor={`objectives.${index}.title`}>Título do Objetivo *</Label>
                                  <Input
                                    id={`objectives.${index}.title`}
                                    placeholder="Ex: Aumentar conversão em 20%"
                                    {...register(`objectives.${index}.title`)}
                                    className={errors.objectives?.[index]?.title ? 'border-red-500' : ''}
                                  />
                                ...
                                <div className="flex items-end space-x-2">
                                  <div className="flex-1">
                                    <Label htmlFor={`objectives.${index}.priority`}>Prioridade *</Label>
                                    <Select
                                      value={watchedObjectives[index]?.priority}
                                      onValueChange={(value) => setValue(`objectives.${index}.priority`, value as any)}
                                    >
                                      <SelectTrigger>
                              ...
                              <div>
                                <Label htmlFor={`objectives.${index}.description`}>Descrição</Label>
                                <Textarea
                              ...
                              <div>
                                <Label htmlFor={`objectives.${index}.success_metric`}>Métrica de Sucesso</Label>
                                <Input
                                  id={`objectives.${index}.success_metric`}
                                  placeholder="Como medir o sucesso? Ex: 20% de aumento, <200ms, 95% satisfação"
                                  {...register(`objectives.${index}.success_metric`)}
                                />
                              </div>
                    ...
                    {typeObjectives.length === 0 && (
                      <div className="text-center py-6">
                        ...
                        <p className="text-gray-600 mb-4">
                          Nenhum objetivo de {type.label.toLowerCase()} adicionado ainda.
                        </p>
                        <Button 
                          type="button" 
                          onClick={() => addObjective(type.value)} 
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Defina os critérios gerais que determinarão o sucesso do projeto como um todo.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Documente limitações de orçamento, tempo, recursos, tecnologia ou outras restrições importantes.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
        ...
        {watchedObjectives.length > 0 && (
          <Card className="bg-gray-50">
            ...
            <CardContent>
              <div className="space-y-4">
                {typeOptions.map((type) => {
                  const objs = getObjectivesByType(type.value);
                  ...
                  return (
                    <div key={type.value}>
                      <div className="flex items-center mb-2">
                        ...
                        <span className="font-medium text-sm text-gray-700">{type.label}:</span>
                      </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-800 space-y-1">
                <li>• Use o método SMART (Específico, Mensurável, Atingível, Relevante, Temporal)</li>
