...
const commonMilestones = [
  {
    ...
    type: 'planning',
    description: 'Reunião inicial e alinhamento da equipe',
    due_date: '',
    status: 'pending' as const,
  },
  {
    title: 'Finalização do Design',
    type: 'planning',
    description: 'Aprovação final dos layouts e protótipos',
    due_date: '',
    status: 'pending' as const,
  },
  {
    ...
    type: 'development',
    description: 'Configuração de repositórios e ambientes',
    due_date: '',
    status: 'pending' as const,
  },
  {
    title: 'MVP Funcional',
    type: 'development',
    description: 'Primeira versão funcional do produto',
    due_date: '',
    status: 'pending' as const,
  },
  {
    ...
    type: 'testing',
    description: 'Testes completos de integração',
    due_date: '',
    status: 'pending' as const,
  },
  {
    ...
    type: 'testing',
    description: 'Validação com usuários finais',
    due_date: '',
    status: 'pending' as const,
  },
  {
    ...
    type: 'deployment',
    description: 'Lançamento oficial do produto',
    due_date: '',
    status: 'pending' as const,
  },
  {
    title: 'Revisão Pós-Launch',
    type: 'review',
    description: 'Análise de métricas e feedback inicial',
    due_date: '',
    status: 'pending' as const,
  },
];
...
export function TimelineStep({ onNext, onBack }: TimelineStepProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      end_date:
        project?.end_date || format(addMonths(new Date(), 3), 'yyyy-MM-dd'),
  ...
  const startDate = watchedData.start_date
    ? parseISO(watchedData.start_date)
    : new Date();
  const endDate = watchedData.end_date
    ? parseISO(watchedData.end_date)
  ...
  const addCommonMilestone = (milestone: MilestoneData) => {
    const exists = watchedData.milestones?.some(
  ...
  const onSubmit = (data: TimelineFormData) => {
    console.log('TimelineStep - onSubmit called with data:', data);
    console.log('TimelineStep - Current project:', project);
    ...
    console.log('TimelineStep - Calling onNext()');
    onNext();
  };
    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-gray-600'>
          Defina o cronograma do projeto, marcos importantes e metodologia de
          desenvolvimento.
        </p>
      </div>
      ...
      <form
        onSubmit={handleSubmit(onSubmit, errors => {
          console.log('TimelineStep - Validation errors:', errors);
        })}
        className='space-y-6'
      >
        {/* Project Timeline */}
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Duração: {projectDuration} dias ({Math.ceil(projectDuration / 7)}{' '}
              semanas)
            </CardDescription>
          </CardHeader>
            ...
            <div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
                {commonMilestones.map(milestone => {
                  ...
                  const typeInfo = getMilestoneTypeInfo(
            ...
            {fields.length === 0 ? (
              <div className='text-center py-8'>
                ...
                <p className='text-gray-600 mb-4'>
                  Adicione marcos importantes para acompanhar o progresso do
                  projeto.
                </p>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                          ...
                          <div className='flex items-end space-x-2'>
                            <div className='flex-1'>
                              <Label htmlFor={`milestones.${index}.type`}>
                                Tipo *
                              </Label>
                              <Select
                                value={milestone?.type}
                                onValueChange={value =>
                                  setValue(`milestones.${index}.type`, value)
                                }
                              >
                                <SelectTrigger>
